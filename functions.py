...
def plot_images(plot_type, time, voltage, i, r, x, ptr, model=None):
    ...
    if plot_type == 1:  # Plots regular resistance plot; Full plot + its local peaks.
        ...
        ax_plot[0].plot(time[ptr:], r[ptr:])  # Solution otherwise.
        ax_plot[0].twinx().plot(time[ptr:], voltage[ptr:], color='r')  # Voltage.
